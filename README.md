...
* 🧩 **Редактор `.env` прямо из бота** (ключи TonAPI/2328, интервалы, whitelist) — секреты маскируются; *в Docker отключен* (переменные задаются в docker-compose)
* 🔄 **OTA-обновления**: проверка новых коммитов и установка с GitHub без SSH (`git pull` + `pip install` + рестарт) — *только при запуске вне Docker*
* 🔁 Перезапуск бота

### 🔎 Инлайн-режим (`@tonancbot запрос`)
* 🧮 **Калькулятор** — `@tonancbot 13 + 13`, `(2+3)*4`, `2^10` (безопасный AST-парсер, без eval)
* 💱 **Конвертер валют и токенов** — `@tonancbot 123 gram rub`, `100 usd rub`, `5 ton usd` — токены (ton, gram, usdt, not, dogs, hmstr) и фиатные валюты (usd, eur, rub, uah, kzt, gbp, cny, jpy, aed, try, byn, pln, chf)
* 💎 **Баланс кошелька** — адрес `UQ...`/`EQ...` или домен `wallet.ton`
* Пустой ввод — карточка вашего кошелька из вотч-листа
* Калькулятор и конвертер доступны **всем пользователям** (визитная карточка бота), курсы кэшируются
...
* Цветные inline-кнопки (`success` / `danger` / `primary`) с премиум-иконками (`icon_custom_emoji_id`)
...
├── emojis.py             # Премиум-эмодзи (custom emoji) + ID иконок кнопок
├── kb.py                 # Хелпер inline-кнопок (цвета + премиум-иконки)
├── inline_tools.py       # Калькулятор и конвертер для инлайн-режима
...
* `@your_bot 13 + 13` — калькулятор
* `@your_bot 123 gram rub` / `@your_bot 100 usd rub` — конвертер валют
